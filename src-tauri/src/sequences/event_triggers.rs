...
impl EventTriggerManager {
    pub fn new() -> Self {
        Self {
            ...
            guard_state: Mutex::new(TriggerGuardState {
        ...
        for def in &definitions {
            for trigger in &def.triggers {
                ...
                } = trigger
                {
                    ...
                    let sequence_id = def.id.clone();
                    let manager_clone = manager.clone();
                    let guard_state = Arc::new(Mutex::new(TriggerGuardState {
                        last_fire: HashMap::new(),
                        daily_count: HashMap::new(),
                    }));
                    let triggered_ids = self.triggered_execution_ids.clone();
                    let cooldown_ms = cooldown.unwrap_or(0);
                    let max_daily = max_per_day.unwrap_or(u32::MAX);
                    ...
                    let event_pattern = match event_type.as_str() {
                        ...
                        "app_start" => {
                            // Fire immediately for app_start (with once_per_day guard)
                            if once_daily {
                                let now = Utc::now().date_naive();
                                let mut gs = guard_state.lock();
                                if let Some((date, _)) = gs.daily_count.get(&trigger_key_clone) {
                                    if *date == now {
                                        continue; // Already fired today
                                    }
                                }
                                gs.daily_count.insert(trigger_key_clone.clone(), (now, 1));
                            }

                            // Fire the sequence
                            let inputs_map: HashMap<String, serde_json::Value> = inputs_clone
                                .clone()
                                .unwrap_or_default()
                                .into_iter()
                            ...
                            continue; // No listener needed for app_start
                        }
                        other => other, // Custom event type
                    };

                    // Register listener for the event
                    let listener_id = app.listen(event_pattern, move |event| {
                        // Check self-exclusion: don't trigger on sessions spawned by sequences
                        let include_seq = filter_clone
                            .as_ref()
                        ...
                        let now = Utc::now();
                        {
                            let mut gs = guard_state.lock();
                            if cooldown_ms > 0 {
                                if let Some(last) = gs.last_fire.get(&trigger_key_clone) {
                            ...
                            if let Some((date, count)) = gs.daily_count.get(&trigger_key_clone) {
