...
> Automation workflows for OpenWhisperer that chain AI operations, git actions, notifications, and human gates into repeatable, schedulable pipelines.
...
nodes:
  ...
  - id: create_pr
    ...
    body: |
      ...
      ---
      *Auto-generated by OpenWhisperer Sequence*
...
```
# All user sequences
~/.config/open-whisperer/sequences/
├── code-review.yaml
...
All sequences are stored in a single global location (`~/.config/open-whisperer/sequences/`). There are no per-repo sequence directories. This keeps things simple — one library, one place to manage.
...
|---------|----------|------------|
| **User sequences** | `~/.config/open-whisperer/sequences/` | Always available (or filtered by `repos` field) |
...
```
You are a workflow generator for OpenWhisperer Sequences.
...
```
~/.config/open-whisperer/sequences/executions/
...
Sequences integrate with OpenWhisperer's existing voice command system.
...
| Feature | OpenWhisperer Sequences | GitHub Actions | Cursor Composer | Aider | Cline/Roo Code |
|---------|---------------------------|---------------|-----------------|-------|----------------|
...
6. **Integrated with OpenWhisperer** — Reuses existing sessions, transcription, repo management, MCP servers
