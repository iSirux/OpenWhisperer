...
- **Setup flow**: starting a run preflights `nm_check` (installed? repo initialized? — the latter detected via the `no-mistakes` git remote that `init` adds). Missing pieces park the run in a `setup` panel state with `setupReason: 'not-installed' | 'not-initialized'`: "Install no-mistakes" (`nm_install` — opens a terminal running the official installer script, Windows via a generated `.cmd` in the config dir), "Initialize repo" (`nm_init` — runs the non-interactive `no-mistakes init` headlessly in the repo), and "Check again" (re-probes; launches the pending run once both pass, or advances the reason). The binary is resolved via `nm_bin()` — the installer's known locations (`%LOCALAPPDATA%\no-mistakes\`, `~/.no-mistakes/bin/`, `~/.local/bin/`) are preferred over PATH so a fresh install works without an app restart.
- **Assumptions**: runs are not persisted across app restart.
