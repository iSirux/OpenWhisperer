...
- `smartQueue.ts` - Smart Queue drain driver: dispatches deferred sessions (`status: 'queued'` → `launchPrepared`) and parked turns (`parkedTurns[]` → `continueRateLimited`) when usage windows reset or a custom `targetStartAt` passes; started once via `startSmartQueue()` in the main layout
...
- `parkedTurns[]` — a live session's pending turns to send, each dispatched via `continueRateLimited(id, turnId)` (mid-run rejection, deferred follow-ups, scheduled turns). Several can be parked at once (e.g. a 09:00 send plus a mid-run rejection); they are ordered oldest-first, dispatched one per drain pass, and **every parked turn additionally waits for its own session to go idle** so a dispatch never interrupts a running agent. Each turn carries a stable `id` pairing it with its ghost bubble (`SdkMessage.queuedTurnId`); the legacy single `rateLimited` slot is migrated into the array on load
