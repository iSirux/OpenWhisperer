<script lang="ts">
  ...
  import { approveNode, rejectNode, retryNode, activeExecutionId } from '$lib/stores/sequenceExecutions';
  ...
  // A prompt node runs as a real, live SDK session (created when the node started).
  let liveSession = $derived(
    node.type === 'prompt'
      ? $sdkSessions.find(
          (s) => s.sequenceNode?.executionId === executionId && s.sequenceNode?.nodeId === node.id
        )
      : undefined
  );
  let canGoToSession = $derived(!!liveSession);

  function goToSession() {
    if (!liveSession) return;
    activeExecutionId.set(null);
    activeSdkSessionId.set(liveSession.id);
    sdkSessions.markAsRead(liveSession.id);
      ...
      {#if canGoToSession}
        <div class="mt-2">
          <button
            ...
            onclick={goToSession}
            title="Jump to this prompt's live session in the main view"
          >
